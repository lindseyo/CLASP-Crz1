function [flag] = ScriptUpdated_gyp7_kinMod_notfit()
flag = 1;
%% DON'T CHANGE STUFF -- this is the script to generate PARTS OF FIGURE 5
% load trajectories - set roffTHR at 0.5
load('20180930_outputs_kineticMod_1state_longerLimited10000.mat','allsGen')
X2 = allsGen;
...
load('20180930_params_kineticMod_limited','paramset')
...
%% THIS IS THE ERROR METRIC - LEAST SQUARES OF THE MEAN
...
% metric for fits
indxxx=find(log(err_endpt) < nanmean(log(err_endpt)) - 1.7*nanstd(log(err_endpt)));
...
indyyy = find(err_transf_1 < nanmean(err_transf_1) - 0*nanstd(err_transf_1));
...
tefGyp = {allsGen_bR(indx_endpt(indx_trans_1(1:100)),1),allsGen_bR(indx_endpt(indx_trans_1(1:100)),17),allsGen_bR(indx_endpt(indx_trans_1(1:100)),18)};

% interpolate
gypa = []; gypt = [];
for gg = 1:length(indx_trans_1(1:100))

function [flag] = ScriptUPDATED_gyp7_coopMod_notfit()
flag = 1;
% load outputs for end point (assume harvest at 5hrs)
load('20180930_outputs_coopMod_1state_longerLimited_paramNum10000.mat','allsGen');
% model time course
getTimes = [0.5:360];
mod_2m6m = allsGen(:,5); % 2m6m
mod_const = allsGen(:,10); % const
% model end point
val = 60; % 5hrs 
...
%%
% load parameters
load('20180930_params_coopMod_limited','paramset')
...
%for i = 1:length(Nmod_trans);
    ...
    %ix = Nscaled_2_endpt(i,:);
    ix = scaled_2_endpt_bR(i,:);
    %ix = scaled_2_endpt_bR(i,[1,5]);
    %iy = Nscaled_C_endpt(i,:);
    iy = scaled_C_endpt_bR(i,:);
    %iy = scaled_C_endpt_bR(i,[1,5]);
 ...
 %   if sum(uppx+lowx) == 4; %10
    indxFits = [indxFits,i];
    end
    
%     if sum(uppx+lowx+uppy+lowy) == 18;
%         indxFits = [indxFits,i];
%     end
...
title(['pGYP7 10k, top',num2str(numel(indxFits)),'Param'])
...
% load data
% load data
dir = 'C:\Users\susanychen\Google Drive\UCSF Graduate Research (El-Samad)\LANS OPTOGENETICS PROJECT\SYC Modeling\20180919_kinMod_paramsearch\dataInput_files\';
load([dir,'20180430_sum3exp_4genes_4tfproms.mat'],'mGyp','sGyp','mPun','sPun','mYps','sYps','mCmk','sCmk');
...
tefGyp = {allsGen_bR(indxFits,1),allsGen_bR(indxFits,17),allsGen_bR(indxFits,18)};

% interpolate
gypa = []; gypt = [];
for gg = 1:length(indxFits)
...
title('Gyp7 Diff Proms - Model');  ylim([0 0.5]); box off; %ylim([0 0.7]); 

function [flag] = Script20181129_updated_3state_ronTHR_samePs_fits()
flag = 1;
%% DON'T CHANGE STUFF -- this is the script to generate PARTS OF FIGURE 5
% load trajectories - set roffTHR at 0.5
load('20181129_3state_ronTHRsamePs_output10000.mat','allsGen')
X2 = allsGen;
...
upp2m = GE2m + sGE2m;
%upp2m = GE2m([1,3]) + sGE2m([1,3]);
low2m = GE2m - sGE2m;
%low2m = GE2m([1,3]) - sGE2m([1,3]);
uppDur = GEdur + sGEdur;
lowDur = GEdur - sGEdur;
...
%meanTransffn=mean(nmod_trans(indx_trans_1(1:100),:)); stdTransffn=std(nmod_trans(indx_trans_1(1:100),:));
...
title(['DoseResp ErrPt8, top',num2str(numel(indyyy)),'Param'])

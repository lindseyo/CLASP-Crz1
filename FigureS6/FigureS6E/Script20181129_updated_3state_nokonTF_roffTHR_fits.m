function [flag] = Script20181129_updated_3state_nokonTF_roffTHR_fits()
flag = 1;
%% DON'T CHANGE STUFF -- this is the script to generate PARTS OF FIGURE 5
% load trajectories - set roffTHR at 0.5
load('20181129_3state_nokonTF_roffTHR_gte_output10000.mat','allsGen')
X1 = allsGen;
load('20181129_3state_nokonTF_roffTHR_2pt7_output10000.mat','allsGen')
X2 = allsGen;
...
%     if sum(uppx+lowx) >= 10; %10
%         indxFits = [indxFits,i];
%     end
    
    if sum(uppx+lowx+uppy+lowy) == 20
        indxFits = [indxFits,i];
    end
...
%%
% directly use to predict the response data
%meanTransffn=mean(mod_trans_bR(indx_endpt(paraNum),:)); stdTransffn=std(mod_trans_bR(indx_endpt(paraNum),:));
meanTransffn=mean(mod_trans_bR(indxFits,:)); stdTransffn=std(mod_trans_bR(indxFits,:));
%figure(3); plot(actiTF,mod_trans_bR(indx_endpt(paraNum),:),'b'); hold on;
figure(3); plot(actiTF,mod_trans_bR(indxFits,:),'k.-','markersize',20,'linewidth',2); hold on;
%shadedErrorBar(actiTF,meanTransffn,stdTransffn,'b'); hold on; 
errorbar(actiTF,expG4,sexpG4,'k.','markersize',20,'linewidth',2); axis tight; title('roffTHR best500 sequ'); ylabel('FITC/SSC'); xlabel('max TF'); 
...
%meanTransffn=mean(nmod_trans(indx_trans_1,:)); stdTransffn=std(nmod_trans(indx_trans_1,:));
figure(3); %plot(actiTF,nmod_trans(indx_trans_1(1:125),:),'b'); hold on; 
...
title(['doseResp ErrTHRpt8 10k, top',num2str(numel(indyyy)),'Param']);
ylabel('FITC/SSC'); xlabel('max TF'); 
legend('data','model'); box off;

%% MODEL PREDICTION - different promoters
% model output%% MODEL PREDICTION - different promoters
% load data
% load data
...
% model end point
val = 60; % 5hrs 
%adhGyp = {allsGen_bR(indxFits(indx_trans_1(1:10)),1),allsGen_bR(indxFits(indx_trans_1(1:10)),4),allsGen_bR(indxFits(indx_trans_1(1:10)),7)};
%rplGyp = {allsGen_bR(indxFits(indx_trans_1(1:10)),1),allsGen_bR(indxFits(indx_trans_1(1:10)),15),allsGen_bR(indxFits(indx_trans_1(1:10)),16)};
tefGyp = {allsGen_bR(indxFits(indyyy),1),allsGen_bR(indxFits(indyyy),17),allsGen_bR(indxFits(indyyy),18)};

% interpolate
gypa = []; gypt = [];
for gg = 1:length(indxFits(indyyy))
...
B = max(gyp(:)); A = min(gyp(:));
iy=[]; iz=[];
for i = 1:length(indxFits(indyyy))
    %b = gypa(:,i);
    c = gypt(:,i);
...
title(['pGYP7 10k, ', num2str(numel(indxFits(indyyy))),'param']);  ylim([0 0.5]); box off; %ylim([0 0.7]); 
